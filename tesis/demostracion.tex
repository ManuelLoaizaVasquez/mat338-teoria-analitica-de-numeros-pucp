...
Al reemplazar $\gamma$ en la f\'ormula desplegada en el p\'arrafo anterior, llegamos a 
$$
\sum_{n = 1}^k \frac{1}{n} - \ln k = \gamma + \int_1^\infty \frac{t - \floor{t}}{t^2} dt - \int_1^\infty \frac{t - \floor{t}}{t^2} dt = 
...
$$
\begin{aligned}
\left|\sum_{n \le x} \frac{1}{n} - \ln x  - \gamma \right| &= \left|\sum_{n \le k} \frac{1}{k}  -\ln k -\gamma \right| + |\ln x -\ln k |  \cr 
& \le \left|   \int_k^\infty \frac{t - \floor{t}}{t^2} dt \right| + \left| \int_k^x  \frac{1}{t}\,dt  \right| \cr 
...
\end{lemma}
\begin{proof}
Esta vez utilizamos la f\'ormula de sumaci\'on de Euler con $f(x) = \ln x$, continua y diferenciable en toda la recta real positiva:  
$$
\begin{aligned}
...
De este modo, al introducir $G(x)=\floor{x}$ en la expresi\'on del p\'arrafo anterior se logra 
$$
...
$$
Una aplicaci\'on directa del lema 3 deriva en 
$$
\sum_{n \leq x} \ln n = \sum_{n \leq x} \Lambda(n) \floor{\frac{x}{n}}. 
$$
De ac\'a, en uso del lema 4 conseguimos
$$
\begin{aligned}
...
\end{aligned}
$$
dado que $\Psi(x) = O(x)$ claramente implica $O(\Psi(x))=O(x)$. 
Si aplicamos el lema 2 al lado izquierdo desembocamos en
$$
...
$$
\begin{aligned}
\sum_{n \leq x} \mu(n) g\left(\frac{x}{n}\right) &= \sum_{n \leq x} \mu(n) \sum_{m \leq x/n} f\left(\frac{x}{nm}\right) \ln \left(x/n  \right) \\
...
$$
Con ello, finalmente, 
utilizamos el lema 6 para concluir lo deseado. 
\end{proof}
...
\begin{proof}
Para utilizar el lema 8, definimos convenientemente 
$$
f(x) = \Psi(x) - x + \gamma + 1.
$$
Antes de aplicar el lema 8, le brindaremos a $g(x) = \sum_{n \leq x} f\left(\frac{x}{n}\right)\ln x$ 
...
\end{aligned}
$$
Analicemos por separado cada sumatoria. 
...
$$
Con esta expresi\'on el lema 9 permite conseguir
$$
...
$$
Merced a los lemas 5 y 7 reducimos la expresi\'on a 
$$
\begin{aligned}
...
\Begin{
    ...
    \For{$i \leftarrow 2$ \KwTo $x$}{
        $suma\_log[i] \leftarrow suma\_log[i - 1] + \log[i]$\;
        $suma\_log^2[i] \leftarrow suma\_log^2[i - 1] + (\log[i])^2$\;
    }
    \textbf{return} $\frac{|suma\_log^2[x] + \text{CalcularSuma}(x) - 2x\ln x|}{x}$\;
}
