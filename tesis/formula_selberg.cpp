...
int BuscarUltimaPosicion(int pos_p, Long x) {
  Long p, left, right;
  left = pos_p;
...
Double CalcularSuma_log_p_log_q(Long x) {
  ...
  for (int pos_p = 0; pos_p < cantidad_primos; pos_p++) {
    ...
    Long p = primos[pos_p];
    Long q = primos[pos_q];
    suma += log(p) * (suma_log_p[q] - suma_log_p[p - 1]);
  }
  return suma;
}
...
Double Selberg(Long x) {
  Double suma_log_p_log_q = CalcularSuma_log_p_log_q(x);
...
int main(void) {
  ...
  for (Long i = 0; i < 19; i++) {
    casos_de_prueba.push_back(1000000 + i * 500000);
  }
  casos_de_prueba.push_back(15000000);  
  casos_de_prueba.push_back(20000000);  
  casos_de_prueba.push_back(25000000);  
  casos_de_prueba.push_back(30000000);  
